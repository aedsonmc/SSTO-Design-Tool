...
simTime = 500; %Simulation Time in Seconds
...
for i = 2:totalIterations
   t = i*timestep;
   ydotdot(i) = (F*sind(phi(i-1))+CL(alpha(i-1))*(0.5*rho(y(i-1))*magu(i-1).^2*A)*cosd(phi(i-1))-CD(alpha(i-1))*(0.5*rho(y(i-1))*magu(i-1).^2)*sind(theta(i-1))-g)/m(i-1);
   y(i) = ydotdot(i)*((t.^2)/2);
   ydot(i) = ydotdot(i)*(t);
   xdotdot(i) = (F*cosd(phi(i-1))-CL(alpha(i-1))*(0.5*rho(y(i-1))*magu(i-1).^2*A)*sind(phi(i-1))-CD(alpha(i-1))*(0.5*rho(y(i-1))*magu(i-1).^2)*cosd(theta(i-1)))/m(i-1);
   x(i) = xdotdot(i)*((t.^2)/2);
   xdot(i) = xdotdot(i)*(t);
...
figure(1)
plot(tplot,x,tplot,xdot,tplot,xdotdot)
title('X')

figure(2)
plot(tplot,y,tplot,ydot,tplot,ydotdot)
title('Y')
